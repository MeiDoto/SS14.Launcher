...
public sealed class OpenUriSecurityTests
{
    ...
    {
        // SafeOpenServerUri only allows http/https, never mailto
        // We test the parse+scheme check logic indirectly
        if (Uri.TryCreate(uri, UriKind.Absolute, out var parsedUri))
        {
            ...
            {
                // mailto is allowed by OpenUri but NOT by SafeOpenServerUri
                isAllowed = false;
            }

            // SafeOpenServerUri should reject anything that isn't http/https
        ...
        {
            // If URI can't be parsed, it's automatically blocked
            Assert.Pass("Invalid URI is automatically rejected");
        }
    }

    [Test]
    public void TestOpenUri_ValidHttpsUri_ParsesCorrectly()
    {
        var uri = "https://spacestation14.io/download";
        Assert.That(Uri.TryCreate(uri, UriKind.Absolute, out var parsed), Is.True);
        Assert.That(parsed!.Scheme, Is.EqualTo("https"));
        Assert.That(parsed.Host, Is.EqualTo("spacestation14.io"));
    }

    [Test]
    public void TestOpenUri_ValidMailtoUri_ParsesCorrectly()
    {
        var uri = "mailto:test@example.com";
        Assert.That(Uri.TryCreate(uri, UriKind.Absolute, out var parsed), Is.True);
        Assert.That(parsed!.Scheme, Is.EqualTo("mailto"));
    }
