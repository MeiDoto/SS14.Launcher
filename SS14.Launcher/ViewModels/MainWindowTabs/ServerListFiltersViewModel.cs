...
public sealed partial class ServerListFiltersViewModel : ObservableObject
{
    ...
    public ServerFilterViewModel FilterRecommendedOnly { get; }
    ...
    public ServerListFiltersViewModel(DataManager dataManager, LocalizationManager loc)
    {
        ...
        FilterRecommendedOnly = new ServerFilterViewModel(
            _loc.GetString("filters-recommended-desc"),
            _loc.GetString("filters-recommended"),
            ServerFilter.Recommended,
            this);
    ...
    public void ApplyFilters(List<ServerStatusData> list)
    {
        ...
        var recommendedOnly = GetFilter(ServerFilter.Recommended);
        ...
        bool DoesServerMatch(ServerStatusData server)
        {
            ...
            if (recommendedOnly)
            {
                if (server.Status == ServerStatusCode.Offline)
                    return false;
                if (server.PlayerCount == 0 && !server.Tags.Contains(Tags.TagEighteenPlus))
                    return false;
                if (server.Ping.HasValue && server.Ping.Value.TotalMilliseconds > 280)
                    return false;
            }
