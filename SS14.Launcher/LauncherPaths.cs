...
public static class LauncherPaths
{
    ...
    public static bool IsPortable => DirUserData.EndsWith("data", StringComparison.OrdinalIgnoreCase);
    ...
    private static string GetUserDataDir()
    {
        ...
        var parentDir = Directory.GetParent(installDir)?.FullName;

        // Портативный режим: если в папке лаунчера или на уровень выше есть portable.txt, .portable или data/
        if (File.Exists(Path.Combine(installDir, "portable.txt")) ||
            File.Exists(Path.Combine(installDir, ".portable")) ||
            Directory.Exists(Path.Combine(installDir, "data")))
        {
            return Path.Combine(installDir, "data");
        }

        if (!string.IsNullOrEmpty(parentDir) &&
            (File.Exists(Path.Combine(parentDir, "portable.txt")) ||
             File.Exists(Path.Combine(parentDir, ".portable")) ||
             Directory.Exists(Path.Combine(parentDir, "data"))))
        {
            return Path.Combine(parentDir, "data");
        }
