...
public sealed partial class Updater : ObservableObject
{
        ...
        CancellationToken cancel)
    {
        ...
        for (var i = 0; i < 5; i++)
        {
            try
            {
                transaction = con.BeginTransaction(deferred: true);
                break;
            }
            catch (SqliteException ex) when (ex.SqliteErrorCode == 5 && i < 4)
            {
                Log.Warning("SQLite busy when beginning transaction, retrying (attempt {Attempt})...", i + 1);
