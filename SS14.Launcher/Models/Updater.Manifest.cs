...
public sealed partial class Updater
{
        ...
        Stopwatch swSqlite)
    {
        ...
        var existingHashes = new HashSet<HashKey>();

        using var stmtAllContent = db.Prepare("SELECT Hash FROM Content");
        while (stmtAllContent.Step(db) == raw.SQLITE_ROW)
        {
            var span = raw.sqlite3_column_blob(stmtAllContent, 0);
            existingHashes.Add(new HashKey(span.ToArray()));
        }
