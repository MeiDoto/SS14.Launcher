...
public sealed class DataManager : ObservableObject
{
    ...
    private void LoadSqliteConfig(SqliteConnection sqliteConnection)
    {
        ...
        foreach (var hub in sqliteConnection.Query<Hub>("SELECT Address,Priority FROM Hub"))
        {
            if (hub.Address.Host.Equals("central.spacestation14.io", StringComparison.OrdinalIgnoreCase))
            {
                sqliteConnection.Execute("DELETE FROM Hub WHERE Address = @Address", new { Address = hub.Address.AbsoluteUri });
                continue;
            }
    ...
    private static SqliteConnection OpenCfgConnection()
    {
        var con = new SqliteConnection(GetCfgDbConnectionString());
        con.Open();
        con.Execute("PRAGMA journal_mode=WAL; PRAGMA synchronous=NORMAL; PRAGMA busy_timeout=30000; PRAGMA temp_store=MEMORY;");
        return con;
    }
    ...
    public async void CommitConfig()
    {
        ...
        await Task.Run(async () =>
        {
            ...
            {
                Log.Error(e, "Failed to commit config to SQLite database");
            }
            finally
            {
    ...
    private static string GetCfgDbConnectionString()
    {
        ...
        return $"Data Source={path};Mode=ReadWriteCreate;Default Timeout=30;Cache=Shared";
    }
    ...
    public void ResetAllCVarsToDefault()
    {
    ...
    public List<ServerHistoryEntry> GetServerHistory()
    {
        try
        {
            var json = GetCVar(CVars.ServerHistory);
            if (string.IsNullOrWhiteSpace(json) || json == "[]")
                return new List<ServerHistoryEntry>();

            return JsonSerializer.Deserialize<List<ServerHistoryEntry>>(json) ?? new List<ServerHistoryEntry>();
        }
        catch (Exception e)
        {
            Log.Error(e, "Failed to deserialize ServerHistory CVar");
            return new List<ServerHistoryEntry>();
        }
    }

    public void AddServerHistoryEntry(string address, string? name = null)
    {
        ...
        try
        {
            ...
            var entryName = !string.IsNullOrWhiteSpace(name) ? name : address;

            var existing = list.FirstOrDefault(x => string.Equals(x.Address, address, StringComparison.OrdinalIgnoreCase));
            if (existing != null)
            {
                ...
                existing.TimeUtc = DateTime.UtcNow;
                list.Remove(existing);
                list.Insert(0, existing);
            }
            else
            {
                list.Insert(0, new ServerHistoryEntry
                {
                    Address = address,
                    Name = entryName,
        ...
        {
            Log.Error(e, "Failed to save ServerHistory entry for {Address}", address);
        }
    }

    public void RemoveServerHistoryEntry(string address)
    {
        try
        {
            var list = GetServerHistory();
        ...
        {
            Log.Error(e, "Failed to remove ServerHistory entry {Address}", address);
        }
    }
    ...
    public Dictionary<string, long> GetServerPlaytime()
    {
        try
        {
            var json = GetCVar(CVars.ServerPlaytime);
        ...
        {
            Log.Error(e, "Failed to deserialize ServerPlaytime CVar");
            return new Dictionary<string, long>(StringComparer.OrdinalIgnoreCase);
        }
    }

    public long GetPlaytimeForServer(string address)
    {
        var dict = GetServerPlaytime();
        return dict.TryGetValue(address, out var seconds) ? seconds : 0;
    }

    public void AddServerPlaytime(string address, long seconds)
    {
        try
        {
            var dict = GetServerPlaytime();
        ...
        {
            Log.Error(e, "Failed to save playtime for {Address}", address);
        }
    }

    public HashSet<string> GetWatchedSlotServers()
    {
        try
        {
            var json = GetCVar(CVars.WatchedSlotServers);
            if (string.IsNullOrWhiteSpace(json))
        ...
        {
            Log.Error(e, "Failed to deserialize WatchedSlotServers CVar");
            return new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        }
    }

    public bool IsSlotServerWatched(string address)
    {
        return GetWatchedSlotServers().Contains(address);
    }

    public void ToggleWatchedSlotServer(string address)
    {
        try
        {
            var set = GetWatchedSlotServers();
            if (!set.Add(address))
            {
                set.Remove(address);
            }
        ...
        {
            Log.Error(e, "Failed to toggle WatchedSlotServers for {Address}", address);
        }
    }
}
