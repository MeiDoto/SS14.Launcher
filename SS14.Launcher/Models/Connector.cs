...
public partial class Connector : ObservableObject
{
        ...
        List<(string, string)> env)
    {
        ...
        // Performance tweaks
        if (_cfg.GetCVar(CVars.EnableTieredPGO))
        {
        ...
        if (_cfg.GetCVar(CVars.MaxPerformanceJit))
        {
        ...
        if (_cfg.GetCVar(CVars.LowLatencyNetworking))
        {
            EnvVar("DOTNET_SYSTEM_NET_SOCKETS_INLINE_COMPLETIONS", "1");
        }

        if (_cfg.GetCVar(CVars.DisableDiagnosticsOverhead))
        {
            EnvVar("DOTNET_EnableDiagnostics", "0");
        }

        if (_cfg.GetCVar(CVars.LowPauseGc))
        {
            EnvVar("DOTNET_GCConcurrent", "1");
            EnvVar("DOTNET_GCLargeObjectHeapCompactionMode", "1");
        }

        if (_cfg.GetCVar(CVars.ForceDedicatedGpu))
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
            {
                ...
                // If NVIDIA proprietary drivers are present, configure NVIDIA render offload
        ...
        if (_cfg.GetCVar(CVars.ProxyEnabled) && _cfg.GetCVar(CVars.ProxyApplyToGameClient))
        {
            var type = _cfg.GetCVar(CVars.ProxyType).ToLowerInvariant();
            var host = _cfg.GetCVar(CVars.ProxyHost);
            var port = _cfg.GetCVar(CVars.ProxyPort);
            var user = _cfg.GetCVar(CVars.ProxyUsername);
            var pass = _cfg.GetCVar(CVars.ProxyPassword);
            ...
            if (!string.IsNullOrWhiteSpace(host))
            {
                ...
                EnvVar("no_proxy", "localhost,127.0.0.1");
                Log.Information("Routing game client traffic through {ProxyScheme} proxy: {ProxyHost}:{ProxyPort}", scheme, host, port);
            }
        }
