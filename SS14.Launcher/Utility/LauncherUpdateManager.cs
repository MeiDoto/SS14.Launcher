...
public sealed class LauncherUpdateManager
{
        ...
        CancellationToken cancel)
    {
        ...
        catch (Exception)
        {
            ...
            {
                Log.Debug(innerEx, "Failed to delete temp file on error {Path}", tempFile);
            }
            ...
            {
                Log.Debug(fallbackEx, "Failed to delete extract directory on error {Path}", extractDir);
            }
            throw;
        }
    }
...
";
            ...
            {
                Log.Debug(ex, "Failed to set unix file mode for script {Path}", scriptPath);
            }
