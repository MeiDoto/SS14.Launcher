...
public class App : Application
{
    ...
    private void LoadBaseAssets()
    {
        foreach (var (name, (path, type)) in AssetDefs)
        {
            ...
            {
                Log.Warning(ex, "Failed to load base asset {Name} from {Path}, trying fallback", name, path);
                if (name == "WindowIcon")
                {
                    try
                    {
                        using var fallbackStream = AssetLoader.Open(new Uri("avares://SS14.Launcher/Assets/icon.png"));
                        var fallbackAsset = LoadAsset(type, fallbackStream);
                        _baseAssets[name] = fallbackAsset;
                        Resources[name] = fallbackAsset;
                    }
                    catch (Exception fallbackEx)
                    {
                        Log.Error(fallbackEx, "Failed to load fallback WindowIcon");
                    }
                }
            }
        }
    ...
    private void OnAssetsChanged(OverrideAssetsChanged obj)
    {
        foreach (var (name, data) in obj.Files)
        {
            ...
            {
                Log.Warning(ex, "Failed to load override asset '{AssetName}', falling back to default", name);
            }
        }
