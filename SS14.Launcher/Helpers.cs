...
public static class Helpers
{
            ...
            try
            {
                do
                {
                    ...
                    else
                    {
                        ...
                        reads += 1;
                        totalRead += read;
                        if (totalLength.HasValue && reads % 10 == 0)
                        {
    ...
    public static void ChmodPlusX(string path)
    {
        ...
        if (OperatingSystem.IsLinux() || OperatingSystem.IsFreeBSD() || OperatingSystem.IsMacOS())
        {
            ...
            {
                Log.Debug(ex, "File.SetUnixFileMode failed for {Path}, falling back to chmod process", path);
            }
            ...
            {
                Log.Warning(ex, "Failed to set execute permissions on {Path}", path);
            }
        }
    }
