...
7. [Подсистема реплеев и автономная загрузка контента](#-подсистема-реплеев-и-автономная-загрузка-контента)
8. [Архитектурные решения (ADR)](#-архитектурные-решения-adr)
...
## 🎬 Подсистема реплеев и автономная загрузка контента

Подсистема управления записями раундов спроектирована для обеспечения максимальной автономности, высокой производительности I/O и полной независимости от конкретных игровых сообществ:
- **`ReplayMetadataCache`**: персистентный JSON-кэш метаданных с инвалидацией по размеру файла и `LastWriteTimeUtc`, исключающий избыточные обращения к диску.
- **Пользовательская загрузка**: поддержка прямых ссылок HTTP/HTTPS и шаблонов ссылок сообществ с параметром `{roundId}` без привязки к сторонним серверам (см. [ADR 0004](adr/0004-replay-subsystem-and-user-architecture.ru.md)).
- **Сверхбыстрый потоковый ввод-вывод**: буферизация 256 КБ на базе `ArrayPool<byte>.Shared`, экспоненциальное сглаживание скорости (EMA, $\alpha=0.35$) и динамический расчет оставшегося времени (ETA).
- **Умная очистка (`SmartReplayCleaner`)**: эвристический анализ квот, возраста записей и целостности zip-архивов с надежной защитой избранного.

---

## 📚 Архитектурные решения (ADR)

- [ADR 0001: Базовая архитектура, модульность и производительность](adr/0001-architecture-decisions.ru.md)
